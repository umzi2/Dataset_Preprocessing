...
class ImgToEmbedding:
    def __init__(self,model:EmbeddedModel=EmbeddedModel.ConvNextS ,amp:bool = True,scale:int=4,device:str="cuda"):
        self.device = torch.device(device)
        self.scale = scale
        self.amp = amp
        self.model = enum_to_model(model).to(self.device)
        self.vit = model in [EmbeddedModel.VITS,EmbeddedModel.VITB,EmbeddedModel.VITL,EmbeddedModel.VITG]
    @staticmethod
    ...
    def img_to_tensor(self,x):
        if self.vit:
            return self.check_img_size(torch.tensor(x.transpose((2, 0, 1)))[None, :, :, :].to(self.device))
    ...
    def __call__(self, x):
        if self.scale >1:
            h,w = x.shape[:2]
            x = resize(x,(w//self.scale,h//self.scale),ResizeFilter.CubicCatrom,False)
