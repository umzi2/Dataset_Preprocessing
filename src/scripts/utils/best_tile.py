...
class BestTile:
    ...
    def __init__(self, in_folder: str, out_folder: str, tile_size: int = 512,
                 process_type: ProcessType = ProcessType.THREAD, scale: int = 1, dynamic_n_tiles:bool=True,
                 median_blur:int = 0, laplacian_thread:float=0, image_gray:bool = False):
        """
    ...
    def get_tile(self,img,laplacian_abs):
        ...
        if self.scale > 1:
            laplacian_abs_r = resize(laplacian_abs, (img_shape[1] // self.scale, img_shape[0] // self.scale),
                                   ResizeFilter.Linear, False).squeeze()
        ...
        laplacian_abs[left_up_cord[0]:left_up_cord[0] + self.tile_size, left_up_cord[1]:left_up_cord[1] + self.tile_size] = -1.0
    ...
    def read_img(self,img_name):
        image = read(os.path.join(self.in_folder, img_name), ImgColor.GRAY if self.image_gray else ImgColor.RGB, ImgFormat.F32)
        return image
    @staticmethod
    ...
    def median_laplacian(self,image):
        if self.median_blur <=5:
            ...
            image = cv2.medianBlur((image*255).astype(np.uint8),self.median_blur).astype(np.float32)/255
    ...
    def process(self, img_name: str):
        ...
        try:
            ...
            if img_shape[0]*img_shape[1]>self.tile_size**2*4 and self.dynamic_n_tiles:
                for i in range((img_shape[0]*img_shape[1])//(self.tile_size**2*2)):
                    ...
                    if self.laplacian_thread:
                        ...
                        if laplacian_tile<self.laplacian_thread:
                            break
                    self.save_result(tile,".".join(img_name.split(".")[:-1])+f"_{i}" + ".png")
            else:
                tile, laplacian_abs = self.get_tile(img,laplacian_abs)
                if self.laplacian_thread:
                    laplacian_tile = np.mean(self.laplacian_image(tile))
