...
class VideoToFrame:
    def __init__(self,embedder:ImgToEmbedding=ImgToEmbedding(),thread:float=0.3,distance_fn=euclid_dist):
        ...
        with tqdm(total=total_frames) as pbar:
            while capture.isOpened():
                ...
                if not ret:
                    break
                if ref is None:
                    ref = self.embedder(cv2.cvtColor(frame,cv2.COLOR_BGR2RGB).astype(np.float32)/255.0)
                else:
                    temp_embedd = self.embedder(cv2.cvtColor(frame,cv2.COLOR_BGR2RGB).astype(np.float32)/255.0)
